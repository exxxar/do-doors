    ...
    @foreach($prices as $index=>$item)
        ...
        <tr style="background: red;">
            ...
            @foreach ($item->prices  as $price)
                ...
                @endphp
                <td width="100px"
                    style="text-align: center;border:1px solid #111;background-color: {{$bg}};">{{ $price->price[$price_type] }}</td>
                <td width="100px"
                    style="text-align: center;border:1px solid #111;background-color: {{$bg}};">{{ $price->price_koef }}</td>
            @endforeach
        </tr>
    @endforeach
    </tbody>
